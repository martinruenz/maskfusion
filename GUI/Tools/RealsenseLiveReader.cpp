...
#include "RealsenseLiveReader.h"

aFreenectLiveReader::aFreenectLiveReader(bool fullResolution) :
    LogReader("", false),
//    lastFrameTime(-1),
//    lastGot(-1),
    ...
    bufferingThread = std::thread(&aFreenectLiveReader::bufferLoop, this);
}

aFreenectLiveReader::~aFreenectLiveReader() {
...
FrameDataPointer aFreenectLiveReader::processNewest() {
...
FrameDataPointer aFreenectLiveReader::getFrameData() { return atomic_load(&currentDataPointer); }

int aFreenectLiveReader::getNumFrames() { return std::numeric_limits<int>::max(); }

bool aFreenectLiveReader::hasMore() { return true; }

void aFreenectLiveReader::bufferLoop(){
