...
void main() {
    ...
    if(colorType0[0] != -1){
        if(colorType0[0] == 1){
            ...
            //ratio = 0.1 * vPosition[0].w; // This would draw confidence
            ...
            // label-ID
            //if(){}
            //if(vHighlighted[0]==1) vColor0 = vec3(1.0f, 1.0f, 0.0f) * max(abs(dot(vNormRad[0].xyz, vec3(1.0, 1.0, 1.0))), 0.8);
            //else if(maskID0[0]==uint(41)) vColor0 = vec3(0.8f, 0.7f, 0.4f) * max(abs(dot(vNormRad[0].xyz, vec3(1.0, 1.0, 1.0))), 0.8);
            //else vColor0 = 0.2 * (vec3(colors[int(mod(maskID0[0],numColors))]) * abs(dot(vNormRad[0].xyz, vec3(1.0, 1.0, 1.0)))) + vec3(0.1f, 0.1f, 0.1f);

            vColor0 = vec3(colors[int(classID0[0])])  * max(abs(dot(vNormRad[0].xyz, vec3(1.0, 1.0, 1.0))), 0.8);
//            if(maskID0[0]==uint(0)) vColor0 = 0.3 * decodeColor(vColor[0].x) + 0.7 * vColor0;


            if(maskID0[0]==uint(0)) {
                // soft color background
                vColor0 = vec3(0.7, 0.7, 0.7) + 0.3 * decodeColor(vColor[0].x) + 0.7 * vColor0;

                // grey
                vColor0 = (vec3(.5f, .5f, .5f) * abs(dot(vNormRad[0].xyz, vec3(1.0, 1.0, 1.0)))) + vec3(0.1f, 0.1f, 0.1f);

                vColor0 = (vec3(.5f, .5f, .5f) * abs(dot(vNormRad[0].xyz, vec3(1.0, 1.0, 1.0)))) + vec3(0.5f, 0.5f, 0.5f);
            }
        }
        else{
            // Grey
