...
//    if(violationCount > 0){ //&& maskID==uint(0)) {
...
//    }
    ...
    if( (maskValue != maskID)  && maskValue < uint(255) && //maskID==uint(0) &&
        (wDepth > localPos.z-0.05 && wDepth < localPos.z+0.05)){
        if(maskValue == uint(0)) vPosition.w *= (0.5 + 0.5 * (1 - outlierCoeff / 10.0)); // penalise objects, from bg
        else if (maskID == uint(0)) vPosition.w *= 0.25 * (0.5 + 0.5 * (1 - outlierCoeff / 10.0)); // penalise bg
        else vPosition.w *= (0.5 + 0.5 * (1 - outlierCoeff / 10.0));
        //else if (maskValue < uint(255)) vPosition.w *= (0.5 + 0.5 * (1 - outlierCoeff / 10.0)); // penalise inter-objects
    }
